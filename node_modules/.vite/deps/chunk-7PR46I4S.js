...
function tokenizeCodeFenced(effects, ok3, nok) {
  ...
  function openAfter(code) {
    ...
    return self.interrupt ? ok3(code) : contentStart(code);
  }
  ...
  function tokenizeNonLazyLine(effects2, ok4, nok2) {
    ...
    function lineStart(code) {
      return self2.parser.lazy[self2.now().line] ? nok2(code) : ok4(code);
    }
  }
...
function tokenizeIndentedContent(effects, ok3, nok) {
  ...
  function afterPrefix(code) {
    const tail = self.events[self.events.length - 1];
    return tail && tail[1].type === types.linePrefix && tail[2].sliceSerialize(tail[1], true).length >= constants.tabSize ? ok3(code) : markdownLineEnding(code) ? start(code) : nok(code);
  }
}
...
function factoryLabel(effects, ok3, nok, type, markerType, stringType) {
  ...
  function atBreak(code) {
    if (code === codes.eof || code === codes.leftSquareBracket || code === codes.rightSquareBracket && !data || code === codes.caret && !size && "_hiddenFootnoteSupport" in self.parser.constructs || size > constants.linkReferenceSizeMax) {
      return nok(code);
    }
...
function tokenizeHeadingAtx(effects, ok3, nok) {
  ...
  function fenceOpenInside(code) {
    ...
    if (code === codes.eof || markdownLineEndingOrSpace(code)) {
      ...
      return self.interrupt ? ok3(code) : headingBreak(code);
    }
    return nok(code);
  }
...
function tokenizeHtmlFlow(effects, ok3, nok) {
  ...
  function cdataOpenInside(code) {
    if (code === buffer.charCodeAt(index++)) {
      effects.consume(code);
      return index === buffer.length ? self.interrupt ? ok3 : continuation : cdataOpenInside;
    }
    return nok(code);
  }
  ...
  function htmlLineEnd(effects2, ok4, nok2) {
    ...
    function lineStart(code) {
      return self.parser.lazy[self.now().line] ? nok2(code) : ok4(code);
    }
  }
...
function tokenizeLabelEnd(effects, ok3, nok) {
  ...
  function afterLabelEnd(code) {
    ...
    return defined ? ok3(code) : balanced(code);
  }
...
//# sourceMappingURL=chunk-7PR46I4S.js.map
